...
void mit_bvt_engine::implement_symbolic_input(uint64_t* context) {
  ...
  // create AST node
  value_v_1[0] = lo; value_v_2[0] = compute_upper_bound(lo, step, up);
    ...
    if (is_valid_virtual_address(vaddr)) {
      if (is_virtual_address_mapped(table, vaddr)) {
        ...
        *(s + i) = *(values + mrvc);
        ...
        if (is_symbolic_value(data_types[mrvc], mintervals_los[mrvc].size(), mintervals_los[mrvc][0], mintervals_ups[mrvc][0], 0)) {
  ...
  if (valid) {
    ...
    if (mrcc > 0) {
      if (is_trace_space_available()) {
        ...
        uint64_t ast_ptr = add_ast_node(CONST, 0, 0, 1, value_v_1, value_v_2, 1, 0, zero_v, MIT, reg_bvts[REG_A0]);
...
void mit_bvt_engine::create_crt_operand_ast_node_entry(uint64_t crt_reg) {
  ...
  if (rd != REG_ZR) {
    ...
    if (reg_symb_type[rs1] == SYMBOLIC) {
      if (reg_symb_type[rs2] == SYMBOLIC) {
        ...
        bool cnd = false;
        for (size_t i = 0; i < reg_mintervals_cnts[rs1]; i++) {
          cnd = mul_condition(reg_mintervals_los[rs1][i], reg_mintervals_ups[rs1][i], multiplier);
          if (cnd == true) {
            reg_mintervals_los[rd][i] = reg_mintervals_los[rs1][i] * multiplier;
            reg_mintervals_ups[rd][i] = reg_mintervals_ups[rs1][i] * multiplier;
          } else {
            std::cout << exe_name << ": cnd failure in mul at " << (uint64_t*) (pc - entry_point) << '\n';
            // upgrade to bvt
            reg_mintervals_los[rd][0] = registers[rd]; // one witness
            reg_mintervals_ups[rd][0] = registers[rd];
            break;
      ...
      bool cnd = false;
      for (size_t i = 0; i < reg_mintervals_cnts[rs2]; i++) {
        cnd = mul_condition(reg_mintervals_los[rs2][i], reg_mintervals_ups[rs2][i], multiplier);
        if (cnd == true) {
...
void mit_bvt_engine::apply_sltu() {
  ...
  if (rd != REG_ZR) {
    ...
    if (reg_data_type[rs1] != POINTER_T) {
      if (reg_data_type[rs2] != POINTER_T) {
        create_sltu_constraints(reg_mintervals_los[rs1], reg_mintervals_ups[rs1], reg_mintervals_los[rs2], reg_mintervals_ups[rs2], mrcc);
      } else
...
void mit_bvt_engine::apply_xor() {
  ...
  if (rd != REG_ZR) {
    ...
    if (reg_data_type[rs1] != POINTER_T) {
      if (reg_data_type[rs2] != POINTER_T) {
        create_xor_constraints(reg_mintervals_los[rs1], reg_mintervals_ups[rs1], reg_mintervals_los[rs2], reg_mintervals_ups[rs2], mrcc);
      } else
        std::cout << exe_name << ": detected pointer data-type in comparison\n";
    } else
      std::cout << exe_name << ": detected pointer data-type in comparison\n";
...
uint64_t mit_bvt_engine::backward_propagation_of_value_intervals(uint64_t ast_tc, std::vector<uint64_t>& lo, std::vector<uint64_t>& up, size_t mints_num, uint8_t theory_type) {
  ...
  switch (ast_nodes[ast_tc].type) {
    ...
    case mit_bvt_engine::REMU: {
      std::cout << exe_name << ": detected an unsupported remu in a conditional expression at 0x" << std::hex << pc - entry_point << std::dec << std::endl;
      exit((int) EXITCODE_SYMBOLICEXECUTIONERROR);
      break;
    }
    default: {
      std::cout << exe_name << ": detected an unknown operation during backward propagation at 0x" << std::hex << pc - entry_point << std::dec << std::endl;
      exit((int) EXITCODE_SYMBOLICEXECUTIONERROR);
      break;
    }
  }
...
void mit_bvt_engine::constrain_memory_mit(uint64_t reg, std::vector<uint64_t>& lo, std::vector<uint64_t>& up, uint32_t mints_num, uint64_t trb, bool only_reachable_branch) {
  uint64_t mrvc;

  if (reg_symb_type[reg] == SYMBOLIC) {
    if (only_reachable_branch == false) {
      backward_propagation_of_value_intervals(reg_asts[reg], lo, up, mints_num, reg_theory_types[reg]);
    }
  }
}
...
void mit_bvt_engine::create_sltu_constraints(std::vector<uint64_t>& lo1_p, std::vector<uint64_t>& up1_p, std::vector<uint64_t>& lo2_p, std::vector<uint64_t>& up2_p, uint64_t trb) {
  bool cannot_handle   = false;
...
void mit_bvt_engine::create_xor_constraints(std::vector<uint64_t>& lo1_p, std::vector<uint64_t>& up1_p, std::vector<uint64_t>& lo2_p, std::vector<uint64_t>& up2_p, uint64_t trb) {
  bool cannot_handle   = false;
  bool true_reachable  = false;
  bool false_reachable = false;
  uint64_t lo1;
...
uint64_t mit_bvt_engine::compute_sub(uint64_t left_operand_ast_tc, uint64_t right_operand_ast_tc, uint64_t old_ast_tc, uint8_t theory_type, uint8_t symbolic_operands) {
  uint64_t sub_lo, sub_up, sub_tmp, ast_ptr;
  uint64_t stored_to_tc, mr_stored_to_tc;
  ...
  if (symbolic_operands == LEFT) {
    ...
    uint64_t resulting_witness = mintervals_los[left_operand_ast_tc][0] - mintervals_ups[right_operand_ast_tc][0];
    propagated_minterval_lo[0] = resulting_witness;
    propagated_minterval_up[0] = resulting_witness;
...
uint64_t mit_bvt_engine::compute_mul(uint64_t left_operand_ast_tc, uint64_t right_operand_ast_tc, uint64_t old_ast_tc, uint8_t theory_type, uint8_t symbolic_operands) {
  ...
  for (size_t i = 0; i < mintervals_los[sym_operand_ast_tc].size(); i++) {
    ...
    if (cnd == true) {
      ...
      std::cout << exe_name << ": detected an overflow case in compute_mul!!! at 0x" << std::hex << pc - entry_point << std::dec << std::endl;
      exit((int) EXITCODE_SYMBOLICEXECUTIONERROR);
    }
  }
