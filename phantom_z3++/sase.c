...
context  ctx;
solver   slv(ctx);
// solver slv = tactic(ctx, "smt").mk_solver();
expr     zero_bv(ctx);
expr     one_bv(ctx);
expr     eight_bv(ctx);
expr     meight_bv(ctx);
expr     zero_int(ctx);   // to trigger an error if used
...
void init_sase() {
  ...
  meight_bv = ctx.bv_val(t, 64);

  zero_int  = ctx.int_val(0);
...
void sase_jal_jalr() {
  if (rd != REG_ZR) {
    // if (rd != 1) // REG_RA
      sase_regs[rd] = ctx.bv_val(pc + INSTRUCTIONSIZE, 64);
    // else
