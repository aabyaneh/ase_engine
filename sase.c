...
// symbolic registers
Z3_ast*   sase_regs;         // array of pointers to SMT expressions
...
Z3_ast*    symbolic_values;

// read trace
uint64_t* concrete_reads;
Z3_ast*   constrained_reads;
uint64_t  read_tc         = 0;
uint64_t  read_tc_current = 0;
uint64_t  read_buffer     = 0;

// ***************** functions related to Z3 solver *****************
...
uint64_t check(Z3_context ctx, Z3_solver s) {
...
void generate_model(Z3_context ctx, Z3_solver s) {
  Z3_model m = 0;
...
void init_sase() {
  ...
  zero_bv     = Z3_mk_unsigned_int64(ctx, 0, bv_sort);
  one_bv      = Z3_mk_unsigned_int64(ctx, 1, bv_sort);

  sase_regs     = malloc(sizeof(Z3_ast)   * NUMBEROFREGISTERS);
  ...
  sase_false_branchs    = malloc(sizeof(Z3_ast)    * sase_trace_size);
  ...
  tcs             = malloc(sase_trace_size * sizeof(uint64_t));
  vaddrs          = malloc(sase_trace_size * sizeof(uint64_t));
  values          = malloc(sase_trace_size * sizeof(uint64_t));
  is_symbolics    = malloc(sizeof(uint8_t)  * sase_trace_size);
  symbolic_values = malloc(sizeof(Z3_ast)   * sase_trace_size);

  concrete_reads        = malloc(sizeof(uint64_t)  * sase_trace_size);
  constrained_reads     = malloc(sizeof(Z3_ast)    * sase_trace_size);
}
...
void sase_lui() {
  if (rd != REG_ZR) {
    sase_regs[rd] = Z3_mk_unsigned_int64(ctx, imm << 12, bv_sort);
...
void sase_addi() {
  if (rd != REG_ZR) {
    sase_regs[rd] = Z3_mk_bvadd(ctx, sase_regs[rs1], Z3_mk_unsigned_int64(ctx, imm, bv_sort));
...
void sase_add() {
  if (rd != REG_ZR) {
    sase_regs[rd] = Z3_mk_bvadd(ctx, sase_regs[rs1], sase_regs[rs2]);
...
void sase_sub() {
  if (rd != REG_ZR) {
    sase_regs[rd] = Z3_mk_bvsub(ctx, sase_regs[rs1], sase_regs[rs2]);
...
void sase_mul() {
  if (rd != REG_ZR) {
    sase_regs[rd] = Z3_mk_bvmul(ctx, sase_regs[rs1], sase_regs[rs2]);
...
void sase_divu() {
  ...
  Z3_solver_assert(ctx, slv, Z3_mk_eq(ctx, sase_regs[rs2], zero_bv));
  if (check(ctx, slv) == 1) {
    printf("%s\n", "SE division by zero!");
    printf("pc: %llx\n", pc - entry_point);
    generate_model(ctx, slv);
  ...
  // divu semantics
  if (rd != REG_ZR) {
    sase_regs[rd] = Z3_mk_bvudiv(ctx, sase_regs[rs1], sase_regs[rs2]);
...
void sase_remu() {
  ...
  // remu semantics
  if (rd != REG_ZR) {
    sase_regs[rd] = Z3_mk_bvurem(ctx, sase_regs[rs1], sase_regs[rs2]);
...
void sase_sltu() {
  ...
  if (rd != REG_ZR) {
    ...
    if (is_branch == 0) {
      ...
      if (is_branch == 2) {
        sase_false_branchs[sase_tc]    = Z3_mk_bvult(ctx, sase_regs[rs1], sase_regs[rs2]);
        sase_pcs[sase_tc]              = pc  + 3 * INSTRUCTIONSIZE;

        Z3_solver_push(ctx, slv);
        Z3_solver_assert(ctx, slv, Z3_mk_bvuge(ctx, sase_regs[rs1], sase_regs[rs2]));
    ...
    if (is_branch == 1) {
      sase_false_branchs[sase_tc]    = Z3_mk_bvuge(ctx, sase_regs[rs1], sase_regs[rs2]);
      sase_pcs[sase_tc]              = pc  + INSTRUCTIONSIZE;

      Z3_solver_push(ctx, slv);
      Z3_solver_assert(ctx, slv, Z3_mk_bvult(ctx, sase_regs[rs1], sase_regs[rs2]));
      pc = pc + INSTRUCTIONSIZE;
    }
...
void sase_backtrack_sltu(int is_true_branch_unreachable) {
  ...
  Z3_solver_pop(ctx, slv, 1);
  Z3_solver_assert(ctx, slv, sase_false_branchs[sase_tc]);
  if (check(ctx, slv) == 0) {
    ...
    if (is_virtual_address_mapped(pt, vaddr)) {
      if (rd != REG_ZR) {
        ...
        if (*(is_symbolics + mrv) == CONCRETE_T) {
          sase_regs_typ[rd] = CONCRETE_T;
          sase_regs[rd]     = Z3_mk_unsigned_int64(ctx, *(values + mrv), bv_sort);
        } else {
          sase_regs_typ[rd] = SYMBOLIC_T;
...
void sase_jal_jalr() {
  if (rd != REG_ZR) {
    sase_regs[rd] = Z3_mk_unsigned_int64(ctx, pc + INSTRUCTIONSIZE, bv_sort);

    sase_regs_typ[rd] = CONCRETE_T;
  }
}

void sase_store_memory(uint64_t* pt, uint64_t vaddr, uint8_t is_symbolic, uint64_t value, Z3_ast sym_value) {
  uint64_t mrv;
