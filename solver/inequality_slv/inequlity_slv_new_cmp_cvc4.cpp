...
int main(int argc, char** argv) {
  ...
  for (unsigned int i = 0; i < strings.size(); i++) {
    ...
    for (unsigned int j = 0; j < strings.at(i).size(); j++) {
      ...
      smt.assertFormula(em.mkExpr(kind::BITVECTOR_UGT, x.back(), e1));
      smt.assertFormula(em.mkExpr(kind::BITVECTOR_ULT, x.back(), e2));
    }
