...
void init_scanner () {
  SYMBOLS = smalloc((SYM_AND + 1) * SIZEOFUINT64STAR);
...
void emit_symbolic_input() {
  create_symbol_table_entry(LIBRARY_TABLE, (uint64_t*) "interval", 0, PROCEDURE, UINT64_T, 0, binary_length);

  emit_ld(REG_A3, REG_SP, 0); // step
  emit_addi(REG_SP, REG_SP, REGISTERSIZE);

  emit_ld(REG_A2, REG_SP, 0); // end
  emit_addi(REG_SP, REG_SP, REGISTERSIZE);

  emit_ld(REG_A1, REG_SP, 0); // start
  emit_addi(REG_SP, REG_SP, REGISTERSIZE);
