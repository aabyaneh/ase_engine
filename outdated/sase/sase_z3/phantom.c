...
void implement_read(uint64_t* context) {
        ...
        if (symbolic) {
          if (sase_symbolic) {
            ...
            if (read_tc_current < read_tc) {
              ...
              Z3_solver_assert(ctx, slv, Z3_mk_bvule(ctx, constrained_reads[read_tc_current], Z3_mk_unsigned_int64(ctx, up, bv_sort)));
              Z3_solver_assert(ctx, slv, Z3_mk_bvuge(ctx, constrained_reads[read_tc_current], Z3_mk_unsigned_int64(ctx, lo, bv_sort)));
              ...
              if (actually_read) {
                ...
                constrained_reads[read_tc] = mk_var(ctx, var_buffer, bv_sort);
                Z3_solver_assert(ctx, slv, Z3_mk_bvule(ctx, constrained_reads[read_tc], Z3_mk_unsigned_int64(ctx, up, bv_sort)));
                Z3_solver_assert(ctx, slv, Z3_mk_bvuge(ctx, constrained_reads[read_tc], Z3_mk_unsigned_int64(ctx, lo, bv_sort)));
  ...
  if (symbolic) {
    if (sase_symbolic) {
      sase_regs[REG_A0] = Z3_mk_unsigned_int64(ctx, *(get_regs(context) + REG_A0), bv_sort);
    } else {
      *(reg_typ + REG_A0) = 0;
...
void implement_brk(uint64_t* context) {
  ...
  if (valid) {
    ...
    if (symbolic) {
      if (sase_symbolic) {
        sase_regs[REG_A0] = Z3_mk_unsigned_int64(ctx, program_break, bv_sort);
      } else {
    ...
    *(get_regs(context) + REG_A0) = program_break;
    ...
    if (symbolic) {
      if (sase_symbolic) {
        sase_regs[REG_A0] = Z3_mk_unsigned_int64(ctx, 0, bv_sort);
      } else {
        *(reg_typ + REG_A0) = 0;
...
void up_load_arguments(uint64_t* context, uint64_t argc, uint64_t* argv) {
  ...
  *(get_regs(context) + REG_SP) = SP;
  ...
  if (symbolic) {
    if (sase_symbolic) {
      sase_regs[REG_SP] = Z3_mk_unsigned_int64(ctx, SP, bv_sort);
    } else {
      *(reg_typ + REG_SP) = 0;
