...
void constrain_mul() {
  ...
  if (rd != REG_ZR) {
    ...
    sase_regs[rd] = boolector_null; // check_smt_expression(MUL);
  ...
  if (reg_mintervals_los[rs2][0] != 0) {
    if (reg_mintervals_ups[rs2][0] >= reg_mintervals_los[rs2][0]) {
      ...
      if (rd != REG_ZR) {
        ...
        sase_regs[rd] = boolector_null; // check_smt_expression(DIVU);
...
void constrain_remu() {
  ...
  sase_regs[rd] = boolector_null; // check_smt_expression(REMU);
...
void constrain_sltu() {
  if (rd != REG_ZR) {
    if (reg_symb_type[rs1] != SYMBOLIC && reg_symb_type[rs2] != SYMBOLIC) {
      ...
      sase_regs[rd]             = (registers[rd] == 0) ? smt_exprs[zero_node] : smt_exprs[one_node]; // zero_bv : one_bv;
      reg_theory_types[rd]      = MIT;
...
uint8_t detect_sym_operand(uint64_t ast_tc) {
  ...
  if (left_typ == SYMBOLIC && right_typ == CONCRETE) {
    ...
    printf("%llu %llu %llu %llu, size: %llu\n", mintervals_los[ast_nodes[ast_tc].left_node][0], mintervals_ups[ast_nodes[ast_tc].left_node][0], left_typ, theory_type_ast_nodes[ast_nodes[ast_tc].left_node], mintervals_los[ast_nodes[ast_tc].left_node].size());
    printf("%llu %llu %llu %llu, size: %llu\n", mintervals_los[ast_nodes[ast_tc].right_node][0], mintervals_ups[ast_nodes[ast_tc].right_node][0], right_typ, theory_type_ast_nodes[ast_nodes[ast_tc].right_node], mintervals_los[ast_nodes[ast_tc].right_node].size());
...
void take_branch(uint64_t b, uint64_t how_many_more) {
  if (how_many_more > 0) {
    ...
    sase_regs[rd]             = (b == 0) ? smt_exprs[zero_node] : smt_exprs[one_node]; // zero_bv : one_bv;
    reg_theory_types[rd]      = MIT;
