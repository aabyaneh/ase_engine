    ...
    *(get_regs(context) + REG_A0) = program_break;
    ...
    if (symbolic) {
      if (symbolic == SASE) {
        ...
        reg_data_type[REG_A0]         = POINTER_T; // VALUE_T;
        ...
        sase_regs[REG_A0]             = (BoolectorNode*) 0; // boolector_unsigned_int(btor, 0, bv_sort); // careful
        reg_theory_types[REG_A0]      = MIT;
      }
    }
  }
  ...
  *(get_regs(context) + REG_SP) = SP;
  ...
  if (symbolic) {
    if (symbolic == SASE) {
      ...
      sase_regs[REG_SP]             = (BoolectorNode*) 0;
