          ...
          if (symbolic == SASE) {
            ...
            if (is_trace_space_available()) {
              ...
              if (mrcc == 0) {
                ...
                uint64_t ast_ptr = add_ast_node(CONST, 0, 0, 1, val_ptr_1, val_ptr_2, 1, 0, zero_v);
                store_symbolic_memory(get_pt(context), vbuffer, value, 0, 0, tc, 1);
              } else {
...
void implement_printsv(uint64_t* context) {
  ...
  if (symbolic && symbolic == MIT) {
    ...
    addr = 0; //(reg_vaddrs_cnts[REG_A1] > 0) ? vaddrs[ld_from_tcs[load_symbolic_memory(get_pt(context), reg_vaddrs[REG_A1][0])][0]] : 0;
...
void implement_symbolic_input(uint64_t* context) {
  ...
  if (symbolic) {
    ...
    if (symbolic == SASE) {
      ...
      reg_mintervals_cnts[REG_A0]   = 0;
      reg_steps[REG_A0]             = 1;
      reg_vaddrs_cnts[REG_A0]       = 0;
      reg_symb_type[REG_A0]         = 0;
      ...
      printf("symbolic input: lo: %llu, up: %llu, step: %llu, cnt: %llu\n", val_ptr_1[0], val_ptr_2[0], step, symbolic_input_cnt);
    ...
    if (symbolic) {
      if (symbolic == SASE) {
        ...
        *(get_regs(context) + REG_A0) = previous_program_break;
        ...
        // printf("malloc: %llu, %llu, %llu\n", reg_data_type[REG_A0], reg_mintervals_los[REG_A0][0], reg_mintervals_ups[REG_A0][0]);
        ...
        if (mrcc > 0) {
          if (is_trace_space_available()) {
            ...
            uint64_t ast_ptr = add_ast_node(CONST, 0, 0, 1, val_ptr_1, val_ptr_2, 1, 0, zero_v);
            store_symbolic_memory(get_pt(context), 0, previous_program_break, POINTER_T, ast_ptr, tc, 1);
          } else {
            throw_exception(EXCEPTION_MAXTRACE, 0);
  ...
  if (symbolic) {
    if (symbolic == MIT) {
      if (is_trace_space_available()) {
        ...
        val_ptr_1[0] = data;
        uint64_t ast_ptr = add_ast_node(CONST, 0, 0, 1, val_ptr_1, val_ptr_1, 1, 0, zero_v);
        store_symbolic_memory(get_pt(context), vaddr, data, 0, ast_ptr, tc, 1);
      } else {
